...
export function Slide6() {
  ...
  return (
    <div className="flex flex-col h-full">
      <h2 className="text-3xl font-bold font-headline text-primary mb-6 animate-in fade-in slide-in-from-top-4 duration-500">Summary & Thank You</h2>
      <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="flex flex-col justify-center animate-in fade-in slide-in-from-left-8 duration-500 delay-200">
          ...
          <ul className="list-disc list-inside space-y-2 text-lg text-muted-foreground">
           ...
           <p className="mt-8 text-2xl font-bold font-headline text-primary animate-in fade-in duration-500 delay-500">
        ...
        <div className="relative rounded-lg overflow-hidden cursor-pointer animate-in fade-in slide-in-from-right-8 duration-500 delay-300" onClick={() => openImage(imageUrl)}>
          <Image src={imageUrl} alt="Thank you" fill style={{ objectFit: 'contain' }} data-ai-hint="friendly handshake" className="transition-transform hover:scale-105" />
        </div>
      </div>
    </div>
  );
